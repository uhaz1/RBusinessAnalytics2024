...
# a vector is created using the combine function c()
...
?sum
sum(sales1) 
min(df$sales)  #min of all elements of df$sales vector
?min
max(df$sales) #max of all elements of df$sales vector
sd(df$radio)  #standard deviation of df$radio vector

length(df$radio) #number of elements in df$radio vector
head(df)
